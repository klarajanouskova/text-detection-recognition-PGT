...
class Batch_Balanced_Dataset(object):
    ...
    def __init__(self, opt):
        ...
        _AlignCollate = AlignCollateNP(imgH=opt.imgH, imgW=opt.imgW, train=True)
        self.data_loader_list = []
        self.dataloader_iter_list = []
        batch_size_list = []
        Total_batch_size = 0
        for selected_d, batch_ratio_d in zip(opt.select_data, opt.batch_ratio):
            if float(batch_ratio_d) == 0:
                continue
            _batch_size = max(round(opt.batch_size * float(batch_ratio_d)), 1)
            log.write(batch_ratio_d + '\n')
            if float(batch_ratio_d) == 0:
                _batch_size = 0
            print(dashed_line)
            log.write(dashed_line + '\n')
...
class LmdbDataset(Dataset):
    ...
    def __init__(self, root, opt, init=True):

        self.root = root

        if not opt.__class__ == dict:
            opt = opt.__dict__
        self.opt = opt
...
def align_images(images, transform, H, W=None, train=False):
    ...
    for image in images:
        ...
        if W is not None and math.ceil(H * ratio) > W:
            resized_w = W
        else:
            resized_w = math.ceil(H * ratio)
        if resized_w > max_w:
            max_w = resized_w
    ...
    image_tensors = torch.cat([t.unsqueeze(0) for t in padded], 0)
    return image_tensors


def pad(img, max_width, transform):
    img = transform(img)
    img = (img - 0.5) * 2
    c, h, w = img.size()
    pad_img = torch.FloatTensor(*[c, h, max_width]).fill_(0)
    pad_img[:, :, :w] = img  # right pad
    # this copies the last column of im pixels to replace 0 padding, do we want this? (may actually serve as a form of aaugmentation, at least shpuld not harm?)
